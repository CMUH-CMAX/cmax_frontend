...
<script>
	...
	let menu_hide = true;
	let symptomsList = { head: [ 1,2,3,4,5,6,7,8,9,0 ], chest: [ 4,5,6 ], left_arm: [ 7,8,9 ], right_arm: [ 10,11,12 ], stomach: [ 13,14,15 ], symphysis: [ 16,17,18 ], left_leg: [ 19,20,21 ], right_leg: [ 22,23,24 ], left_knee: [ 25,26,27 ], right_knee: [ 28,29,30 ], left_calves: [ 31,32,33 ], right_calves: [ 34,35,36 ], left_foot: [ 37,38,39 ], right_foot: [ 39,40,41 ], }
	...
	function select_symptoms( ){
		...
		if(symptoms_selected[hash] != undefined){
			...
			symptoms_selected[hash] = {
				part: body_selected, symptom: this.dataset.name
			}
			// symptoms_selected_count+=1;
		}
...
<div class="process bottom-0 border-t">
  <div class="selection rounded-t-lg {body_selected === undefined ? 'hide-menu' : 'show-menu'} {menu_hide ? 'not-ready' : ''}">
    ...
    <div class="selection-body z-0">
      {#each symptomsList[body_selected] as symptom}
        ...
        <div class="selection-item p-5 border-b text-base symptom-select-{symptoms_selected[encodeURIComponent(body_selected+symptom)]!=undefined}" on:click={select_symptoms} data-name="{symptom}">
          {symptom}
        </div>
      {/each}
    </div>
    ...
    <div class="grid grid-cols-2 mb-4">
      ...
      <div class="grid-span-1 text-right text-xs">清空症狀</div>
		...
		<a href="/search/result">
			<div class="next-step-{symptoms_selected_pass} text-center p-2 rounded-lg">
				下一步
			</div>
		</a>
  </div>
