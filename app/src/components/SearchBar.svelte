...
<div class={twMerge("flex", extraSpacing)}>
    <label class={twMerge("flex relative bg-white rounded-full text-xs py-2 col-span-full flex-1",
{
...
{#if return_btn}
<button on:click={() => {history.back()}} class='ml-4'>取消</button>
{/if}
</div>